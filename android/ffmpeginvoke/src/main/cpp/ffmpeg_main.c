...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time) {
    ...
    /* start for progress */
    if(is_last_report) {
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "  progress=100%%");
        av_bprintf(&buf_script, "progress=100%%\n");
    } else {
        int64_t original_duration = 0;
        if (input_files[0] != NULL && input_files[0]->ctx != NULL) {
            original_duration = input_files[0]->ctx->duration;
        }
        if (original_duration <= 0) {
            original_duration = INT_MAX;
        }
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "  progress=%3d%%", (int)(pts * 100 / original_duration));
        av_bprintf(&buf_script, "progress=%3d%%\n",  (int)(pts * 100 / original_duration));
    }
    /* end for progress */
...
static int check_decode_result(InputStream *ist, int *got_output, int ret) {
    if (*got_output || ret < 0)
...
//     }
    ...
    return exit_program(received_nb_signals ? 255 : main_return_code);
}
